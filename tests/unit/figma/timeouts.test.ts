...
describe('WebSocket timeout constants', () => {
  it('are positive integers', () => {
    for (const v of [WS_COMMAND_DEFAULT_TIMEOUT_MS, WS_STALL_DETECTION_MS, WS_REFRESH_VARIABLES_TIMEOUT_MS]) {
      expect(Number.isInteger(v)).toBe(true);
      expect(v).toBeGreaterThan(0);
    }
  });

  it('preserve ordering: COMMAND_DEFAULT < STALL_DETECTION < REFRESH_VARIABLES', () => {
