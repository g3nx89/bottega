...
describe('FigmaWebSocketServer', () => {
  ...
  // ==========================================================================
  // Grace period + pending requests
  // ==========================================================================

  describe('grace period + pending requests', () => {
    it('pending command is rejected on reconnect, but server stays healthy', async () => {
      await client.connect(port, 'file-R', 'Reconnect File');

      // Send a command with no handler — it will stay pending
      const cmdPromise = server.sendCommand('SLOW_OP', { x: 1 }, 10000);
      // Immediately attach a catch so the rejection is handled before it fires
      const rejectionResult = cmdPromise.catch((err: Error) => err.message);

      await client.close();
      await new Promise((r) => setTimeout(r, 100));

      // Reconnect within 5s grace period with same fileKey
      // handleFileInfo calls rejectPendingRequestsForFile for the old connection
      const client2 = new WsTestClient();
      await client2.connect(port, 'file-R', 'Reconnect File');
      client2.onCommand('SLOW_OP', () => ({ done: true }));

      // The old pending request was rejected when the new client replaced it
      const errMsg = await rejectionResult;
      expect(errMsg).toContain('replaced');

      // Server is healthy — new commands work fine
      const freshResult = await server.sendCommand('SLOW_OP', { x: 2 }, 5000);
      expect(freshResult).toEqual({ done: true });
      expect(server.isClientConnected()).toBe(true);

      await client2.close();
    });

    it('pending request rejects after grace period expires', async () => {
      ...
      try {
        ...
        const errMsg = await rejectionResult;
        expect(errMsg).toContain('disconnected');
      } finally {
        vi.useRealTimers();
      }
    });
  ...
  describe('rapid reconnection stress', () => {
    it('5 rapid disconnect/reconnect cycles keep server consistent', async () => {
      let currentClient = new WsTestClient();
      await currentClient.connect(port, 'file-stress', 'Stress File');

      for (let i = 0; i < 5; i++) {
        await currentClient.close();
        await new Promise((r) => setTimeout(r, 20));

        currentClient = new WsTestClient();
        await currentClient.connect(port, 'file-stress', 'Stress File');
      }

      // After 5 cycles the server should be perfectly healthy
      expect(server.isClientConnected()).toBe(true);
      expect(server.getActiveFileKey()).toBe('file-stress');

      const files = server.getConnectedFiles();
      expect(files.length).toBe(1);
      expect(files[0].fileKey).toBe('file-stress');

      // Verify a command round-trips successfully
      currentClient.onCommand('PING', () => ({ pong: true }));
      const result = await server.sendCommand('PING', {}, 5000);
  ...
  // ==========================================================================
  // Grace period + activeFileKey fallback
  // ==========================================================================

  describe('grace period + activeFileKey fallback', () => {
    it('activeFileKey falls back when active file disconnects after grace period', async () => {
      vi.useFakeTimers({ shouldAdvanceTime: true });
      try {
        // Connect two clients
        const clientA = new WsTestClient();
        await clientA.connect(port, 'file-A', 'File A');
        const clientB = new WsTestClient();
        await clientB.connect(port, 'file-B', 'File B');

        // Make file-A the active file via SELECTION_CHANGE
        clientA.sendEvent('SELECTION_CHANGE', {
          nodes: [{ id: '1:1', name: 'Rect', type: 'RECTANGLE' }],
          count: 1,
          page: 'Page 1',
          timestamp: Date.now(),
        ...
        // After grace period (5s), file-A entry is removed and active falls back
        await vi.advanceTimersByTimeAsync(5500);

        // activeFileKey should fall back to file-B (the only remaining open connection)
        expect(server.getActiveFileKey()).toBe('file-B');
        expect(server.isClientConnected()).toBe(true);
