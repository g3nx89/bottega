...
description: Debug, develop, and interactively test the Bottega Electron app — launch via Playwright, inspect UI, read pino logs, take screenshots, check WebSocket bridge, and run the fix-rebuild-verify cycle. Use PROACTIVELY when debugging runtime errors, investigating UI or IPC issues, testing features end-to-end, verifying fixes visually, or when the user says "run the app", "debug this", "check if it works", "test it", "launch and see", "what does the UI show", "inspect the app", "try the app", "reproduce the bug". Also use when the user reports a crash, blank screen, connection issue, or any runtime behavior problem.
---
